...
if args.train:
    ...
    for step in range(1, args.samples + 1):
        ...
        for i in ks:
            accuracies_k,_ = torch.max(accuracies[:,:i], dim=1)
            accuracy_k = torch.mean(accuracies_k).item()
            writer.add_scalar('accuracy/@'+str(i), accuracy_k, step)
...
def explain(line,filename,explain_type):
    ...
    if explain_type=='char':
        ...
        for word_length in word_lengths:
            line_tensor_copy = line_tensor.clone().detach()
            line_tensor_copy[count:count+word_length,0,:] = torch.zeros([word_length,len(vocabulary)])
            word_output,_ = model(line_tensor_copy)
            word_probs = softmax(word_output)
            word_l2_norm = torch.norm(probs-word_probs)**2

            for i in range(count,count+word_length):
                scores[i-1]=word_l2_norm
            count = count+word_length+1
...
if args.embed:
    ...
    print('-----------------------------------------------------------------------')
    print('model.fc.weight.shape=', model.fc_class.weight.shape)
    print('model.fc.bias.shape=', model.fc_class.bias.shape)
